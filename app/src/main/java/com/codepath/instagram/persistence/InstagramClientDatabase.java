...
public class InstagramClientDatabase extends SQLiteOpenHelper {
  ...
  public void addInstagramPosts(List<InstagramPost> posts) {
    if (posts == null) {
      throw new IllegalArgumentException(
              String.format("Attemping to add a null list of posts to %s", DATABASE_NAME));
    }
  ...
  // Poor man's "upsert".
  // Since SQLite doesn't support "upsert" we need to fall back on an attempt to UPDATE
  // (in case the user already exists, followed by an INSERT (in case the user
  // does not already exist).
  // Unfortunately, there is a bug with the insertOnConflict method
  // (https://code.google.com/p/android/issues/detail?id=13045) so we need to fall back
  ...
  private long addorUpdateUser(InstagramUser user) {
    if (user == null) {
      throw new IllegalArgumentException(
              String.format("Attemping to add a null user to %s", DATABASE_NAME));
    }
    ...
    if (rows == 1) {
      ...
      String usersSelectQuery = String.format(
      ...
      Cursor usersCursor =
              db.rawQuery(usersSelectQuery, new String[]{String.valueOf(user.userName)});
      try {
  ...
  private long addImage(InstagramImage image) {
    if (image == null) {
      throw new IllegalArgumentException(
              String.format("Attemping to add a null image to %s", DATABASE_NAME));
    }
  ...
  private long addComment(InstagramComment comment, long postId) {
    if (comment == null) {
      throw new IllegalArgumentException(
              String.format("Attemping to add a null comment to %s", DATABASE_NAME));
    }
    SQLiteDatabase db = getWritableDatabase();
  ...
  public List<InstagramPost> getAllInstagramPosts() {
    ...
    String imageJoin = String.format(
            LEFT_OUTER_JOIN_FORMAT_STRING,
            TABLE_IMAGES,
            TABLE_POSTS,
    ...
    String commentJoin = String.format(
            LEFT_OUTER_JOIN_FORMAT_STRING,
            TABLE_COMMENTS,
            TABLE_POST_COMMENTS,
            KEY_POST_COMMENT_COMMENT_ID_FK,
            TABLE_COMMENTS,
            KEY_COMMENT_ID);
    String commentUserJoin = String.format(
            LEFT_OUTER_JOIN_FORMAT_STRING,
            TABLE_USERS,
            TABLE_USERS,
            KEY_USER_ID,
    ...
    String commentsSelectQuery = String.format(
        ...
        do {
          ...
          post.mediaId =
                  postsCursor.getString(
                          postsCursor.getColumnIndexOrThrow(KEY_POST_MEDIA_ID));
          post.caption =
                  postsCursor.getString(
                          postsCursor.getColumnIndexOrThrow(KEY_POST_CAPTION));
          post.likesCount =
                  postsCursor.getInt(
                          postsCursor.getColumnIndexOrThrow(KEY_POST_LIKES_COUNT));
          post.commentsCount = postsCursor.getInt(
                  postsCursor.getColumnIndexOrThrow(KEY_POST_COMMENTS_COUNT));
          post.createdTime = postsCursor.getLong(
                  postsCursor.getColumnIndexOrThrow(KEY_POST_CREATED_TIME));

          InstagramUser user = new InstagramUser();
          user.userName = postsCursor.getString(
                  postsCursor.getColumnIndexOrThrow(KEY_USER_NAME));
          user.profilePictureUrl = postsCursor.getString(
                  postsCursor.getColumnIndexOrThrow(KEY_USER_PROFILE_PICTURE_URL));
          post.user = user;

          InstagramImage image = new InstagramImage();
          image.imageUrl = postsCursor.getString(
                  postsCursor.getColumnIndexOrThrow(KEY_IMAGE_URL));
          image.imageHeight = postsCursor.getInt(
                  postsCursor.getColumnIndexOrThrow(KEY_IMAGE_HEIGHT));
          image.imageWidth = postsCursor.getInt(
                  postsCursor.getColumnIndexOrThrow(KEY_IMAGE_WIDTH));
          post.image = image;
          ...
          try {
            if (commentsCursor.moveToFirst()) {
              do {
                ...
                comment.text = commentsCursor.getString(
                        commentsCursor.getColumnIndexOrThrow(
                                KEY_COMMENT_TEXT));
                comment.createdTime = commentsCursor.getLong(
                        commentsCursor.getColumnIndexOrThrow(
                                KEY_COMMENT_CREATED_TIME));

                InstagramUser commentUser = new InstagramUser();
                commentUser.userName = commentsCursor.getString(
                        commentsCursor.getColumnIndexOrThrow(KEY_USER_NAME));
                commentUser.profilePictureUrl = commentsCursor.getString(
                        commentsCursor.getColumnIndexOrThrow(
                                KEY_USER_PROFILE_PICTURE_URL));
