package com.codepath.instagram.database;
...
public class InstagramClientDatabase extends SQLiteOpenHelper {
    ...
    private static final String TAG = InstagramClientDatabase.class.getSimpleName();
    private static final String DATABASE_NAME = "instagramclient.db";
    ...
    private static String LEFT_OUTER_JOIN_FORMAT_STRING = "LEFT OUTER JOIN %s ON %s.%s = %s.%s";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(createPostsCommentTable());
    }
    ...
    private String createImagesTable() {
        return "CREATE TABLE " + TABLE_IMAGES + "(" +
                KEY_ID + " INTEGER PRIMARY KEY," +
                KEY_IMAGE_URL + " TEXT," +
    ...
    private String createPostsCommentTable() {
        return  "CREATE TABLE " + TABLE_POST_COMMENTS + "(" +
    ...
    public synchronized void clearDatabase() {
        ...
        dropIfExists(TABLE_USERS, db);
        dropIfExists(TABLE_IMAGES, db);
        dropIfExists(TABLE_COMMENTS, db);
        dropIfExists(TABLE_POSTS, db);
        dropIfExists(TABLE_POST_COMMENTS, db);
        closeDatabase();
    }

    public void dropIfExists(String tableName, SQLiteDatabase writeableDatabase) {
        if (existsTable(tableName, writeableDatabase)) {
            writeableDatabase.execSQL("DELETE FROM " + tableName);
        }
    }

    public synchronized void replacePosts(List<InstagramPost> posts) {
        clearDatabase();
    ...
    public synchronized long addPost(InstagramPost post) {
        SQLiteDatabase db = getWritableDatabase();
    ...
    private long addComment(InstagramComment comment, long postId, long userId) {
        if (comment != null) {
            SQLiteDatabase db = getWritableDatabase();
